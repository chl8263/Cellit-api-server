...
class AccountContext(username: String?, password: String?, authorities: List<SimpleGrantedAuthority>)
    : User(username, password, authorities) {

    companion object{
        fun fromAccountModel(account: Account): AccountContext{
            return AccountContext(account.accountname, account.password, parseAuthorities(account.role))
        }
